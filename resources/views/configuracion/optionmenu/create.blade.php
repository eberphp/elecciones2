...
<div class="modal fade modal-slide-in-center" aria-hidden="true" role="dialog" tabindex="-1" id="modal-add">
    <div class="modal-dialog ">
        <div class="modal-content">
            ...
            {!! Form::open(['url' => 'configuracion/optionmenu', 'method' => 'POST', 'autocomplete' => 'off', 'files' => 'true']) !!}
            {!! Form::token() !!}
            <div class="modal-body">
                <div class="form-group">
                    ...
                    <div class="input-group a mb-3">
                        ...
                        <input type="text" class="form-control fi" name="codOption" placeholder="Ingrese cod. Menu"
                            autocomplete="off" value="{{ old('codOption') }}">
                    </div>
                    {!! $errors->first('codOption', '<span class="help-block text-danger"><b>:message </b></span>') !!}
                </div>
               
                    <div class="form-group">
                        <label>Seleccione Cod. Menu<span class="text-danger" title="Campo obligatorio">*</span></label>
                        <div class="input-group a mb-3">
                            <div class="input-group-prepend">
                                <span class="input-group-text"><span
                                        class="fas fa-box-open fa-2x-x"></span></span>
                            </div>
                            <select class="form-control fi selectpicker" name="idMenu" id="idMenu"  data-live-search="true" required>
                                {{-- <select name="idClient" id="" class="form-control selectpicker" data-live-search="true"> --}}
                                    <option value=" " selected hidden>Seleccionar</option>
                                @foreach ($menu as $c)
                                    @if (old('idMenu') == $c->idMenu)
                                        <option value="{{ $c->idMenu }}" selected>
                                            {{ $c->codMenu }}
                                        </option>
                                    @else
                                        <option value="{{ $c->idMenu }}"> {{ $c->codMenu}}
                                        </option>
                                    @endif
                                @endforeach
                            </select>
