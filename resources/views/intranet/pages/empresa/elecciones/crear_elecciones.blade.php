                        ...
                        <table class="table table-flush" id="tbData">
                            <thead class="thead-light">
                                <tr>
                                    ...
                                    <th style="font-size: .65rem;">Nombre</th>
                            ...
                            <tbody>
                                @foreach ($elecciones as $eleccion)
                                    <tr style="font-size: 14px;color:black;">
                                        ...
                                        <td><span
                                                class="badge badge-md bg-gradient-success">{{ $eleccion->fecha_inicio }}</span>
                                        </td>
                                        <td><span
                                                class="badge badge-md bg-gradient-dark">{{ $eleccion->fecha_termino }}</span>
                                        </td>
                                        <td>{{ $eleccion->observaciones }}</td>
                                        <td>
                                            @if ($eleccion->encuesta_manual == 'Si')
                                                <span
                                                    class="badge badge-md bg-gradient-success">{{ $eleccion->encuesta_manual }}</span>
                                            @else
                                                <span
                                                    class="badge badge-md bg-gradient-danger">{{ $eleccion->encuesta_manual }}</span>
                                            @endif
                                        </td>
                                        <td>
                                            @if ($eleccion->estado == 'Activo')
                                                <span
                                                    class="badge badge-md bg-gradient-success">{{ $eleccion->estado }}</span>
                                            @else
                                                <span
                                                    class="badge badge-md bg-gradient-danger">{{ $eleccion->estado }}</span>
                                            @endif
                                        </td>
                                        <td>
                                            <div class="d-flex align-items-center">
                                                ...
                                                @if (date('Y-m-d') <= $eleccion->fecha_termino)
                                                    @if ($eleccion->encuesta_manual == 'Si')
                                                        <a href="{{ route('elecciones_voto.manual', ['eleccion' => $eleccion->id]) }}"
        ...
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                <div class="modal-header bg-success">
                    <h5 class="modal-title text-white" id="exampleModalLabel">Crear Elección</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                <form action="{{ route('elecciones.store') }}" method="post" id="forms"
...
@section('script')
    ...
    <script>
                ...
                fetch('/elecciones/' + ids + '/show', {
                    ...
                    .then((response) => {
                        if (response.status) {
                            ...
                            $("#forms")[0].attributes[0].value = '/elecciones/' + response.data.id +
                                '/update';
                            $("#forms")[0].attributes[1].value = 'POST';
                            $("#forms input[name=id]").val(response.data.id);
                            $("#forms input[name=nombre]").val(response.data.nombre);
                            $("#forms input[name=inicio]").val(response.data.fecha_inicio.split(" ")[0]);
                            $("#forms input[name=termino]").val(response.data.fecha_termino.split(" ")[0]);
        ...
        $('#exampleModal').on('hidden.bs.modal', function(e) {
            ...
            $("#forms")[0].attributes[0].value = '/elecciones';
            $("#forms")[0].attributes[1].value = 'POST';
        });
    </script>
    <script>
